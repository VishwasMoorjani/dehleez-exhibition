            ...
            <table class="table table-flush" id="contacts-list">
              <thead>
                <tr>
                  ...
                  <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Booked By</th>
              ...
              <tbody>
                ...
                foreach ($orders as $contact) { ?>
                  <tr>
                    ...
                    <td>
                      <p class="text-xs font-weight-bold mb-0"><?= $contact['booked_by']; ?></p>
                    </td>
                    <td><p class="text-xs font-weight-bold mb-0">
                        <?php
                            $today = date('Y-m-d');
                            foreach ($exhibitions as $exhibitionss) {
                              if ($contact['exhibition_id'] == $exhibitionss['id']) {
                                if($exhibitionss['start_date'] > $today ){ echo "upcoming"; }
                                elseif($exhibitionss['start_date'] < $today && $exhibitionss['end_date'] > $today){ echo "Ongoing"; }
                                elseif($exhibitionss['end_date'] < $today){ echo "Completed"; } 
                              }
                            }
                        ?>
                        </p>
                    ...
                    <td>
                        <?php
                            $today = date('Y-m-d');
                            foreach ($exhibitions as $exhibitionsss) {
                              if ($contact['exhibition_id'] == $exhibitionsss['id']) {
                                if($exhibitionsss['start_date'] > $today ){  ?>
                                    <a onclick="return confirm('Are you sure to delete item?')?window.location.href='<?= base_url('admin/cancelorder/' . $contact['id']); ?>':false;" class="text-secondary font-weight-bold text-xs" data-toggle="tooltip" data-original-title="Delete exhibition">
